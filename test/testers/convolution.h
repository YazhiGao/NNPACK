...
class ConvolutionTester {
 public:
  ...
  inline float errorLimit() const { return this->errorLimit_; }
  ...
  inline bool multithreading() const { return this->multithreading_; }
  ...
  inline size_t batchSize() const { return this->batchSize_; }
  ...
  inline size_t inputChannels() const { return this->inputChannels_; }
  ...
  inline size_t outputChannels() const { return this->outputChannels_; }
  ...
  inline struct nnp_size inputSize() const { return this->inputSize_; }

  inline size_t inputHeight() const { return this->inputSize_.height; }

  inline size_t inputWidth() const { return this->inputSize_.width; }
  ...
  inline struct nnp_size kernelSize() const { return this->kernelSize_; }

  inline size_t kernelHeight() const { return this->kernelSize_.height; }

  inline size_t kernelWidth() const { return this->kernelSize_.width; }
  ...
  inline size_t outputHeight() const {
    return (this->inputPadding_.top + this->inputSize_.height +
            this->inputPadding_.bottom - this->kernelSize_.height) /
               this->outputSubsampling_.height +
           1;
  }

  inline size_t outputWidth() const {
    return (this->inputPadding_.left + this->inputSize_.width +
            this->inputPadding_.right - this->kernelSize_.width) /
               this->outputSubsampling_.width +
           1;
  }
  ...
  inline ConvolutionTester& inputPadding(size_t top, size_t right,
                                         size_t bottom, size_t left) {
  ...
  inline struct nnp_padding inputPadding() const { return this->inputPadding_; }
  ...
  void testOutput(
    ...
    std::vector<float> input(batchSize() * inputChannels() * inputHeight() *
                             inputWidth());
    ...
    std::vector<float> output(batchSize() * outputChannels() * outputHeight() *
                              outputWidth());
    ...
    enum nnp_status status = nnp_convolution_output(
        algorithm, batchSize(), inputChannels(), outputChannels(), inputSize(),
        inputPadding(), kernelSize(), nullptr, nullptr, nullptr, nullptr,
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      nnp_convolution_output__reference(
          batchSize(), inputChannels(), outputChannels(), inputSize(),
          inputPadding(), kernelSize(), outputSubsampling(), input.data(),
          kernel.data(), bias.data(), referenceOutput.data(), this->threadpool);
      ...
      enum nnp_status status = nnp_convolution_output(
          algorithm, batchSize(), inputChannels(), outputChannels(),
          inputSize(), inputPadding(), kernelSize(), input.data(),
          kernel.data(), bias.data(), output.data(),
          scratchSize == 0 ? nullptr : scratchBuffer.data(),
          scratchSize == 0 ? nullptr : &scratchSize, activation, nullptr,
          this->threadpool, nullptr);
      ASSERT_EQ(nnp_status_success, status);

      const float maxError = std::inner_product(
          referenceOutput.cbegin(), referenceOutput.cend(), output.cbegin(),
          0.0f, [](float x, float y) -> float { return std::max<float>(y, x); },
          relativeError);
  ...
  void testInputGradient(
    ...
    std::vector<float> outputGradient(batchSize() * outputChannels() *
                                      outputHeight() * outputWidth());
    std::vector<float> kernel(outputChannels() * inputChannels() *
                              kernelHeight() * kernelWidth());

    std::vector<float> inputGradient(batchSize() * inputChannels() *
                                     inputHeight() * inputWidth());

    std::vector<float> referenceInputGradient(batchSize() * inputChannels() *
                                              inputHeight() * inputWidth());
    ...
    enum nnp_status status = nnp_convolution_input_gradient(
        algorithm, batchSize(), inputChannels(), outputChannels(), inputSize(),
        inputPadding(), kernelSize(), nullptr, nullptr, nullptr, nullptr,
        &scratchSize, nnp_activation_identity, nullptr, this->threadpool,
        nullptr);
    ASSERT_EQ(nnp_status_success, status);
    ...
    std::vector<float> maxErrors;
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      std::generate(outputGradient.begin(), outputGradient.end(),
                    std::ref(rng));
      std::generate(kernel.begin(), kernel.end(), std::ref(rng));
      ...
      nnp_convolution_input_gradient__reference(
          batchSize(), inputChannels(), outputChannels(), inputSize(),
          inputPadding(), kernelSize(), outputGradient.data(), kernel.data(),
          referenceInputGradient.data(), this->threadpool);

      enum nnp_status status = nnp_convolution_input_gradient(
          algorithm, batchSize(), inputChannels(), outputChannels(),
          inputSize(), inputPadding(), kernelSize(), outputGradient.data(),
          kernel.data(), inputGradient.data(),
          scratchSize == 0 ? nullptr : scratchBuffer.data(),
          scratchSize == 0 ? nullptr : &scratchSize, nnp_activation_identity,
          NULL, this->threadpool, nullptr);
      ASSERT_EQ(nnp_status_success, status);

      const float maxError = std::inner_product(
          referenceInputGradient.cbegin(), referenceInputGradient.cend(),
          inputGradient.cbegin(), 0.0f,
          [](float x, float y) -> float { return std::max<float>(y, x); },
          relativeError);
      maxErrors.push_back(maxError);
    }
    EXPECT_LT(median(maxErrors), errorLimit());
  ...
  void testKernelGradient(
    ...
    std::vector<float> outputGradient(batchSize() * outputChannels() *
                                      outputHeight() * outputWidth());
    std::vector<float> kernelGradient(outputChannels() * inputChannels() *
                                      kernelHeight() * kernelWidth());

    std::vector<float> referenceKernelGradient(
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      nnp_convolution_kernel_gradient__reference(
          batchSize(), inputChannels(), outputChannels(), inputSize(),
          inputPadding(), kernelSize(), input.data(), outputGradient.data(),
          referenceKernelGradient.data(), this->threadpool);

      enum nnp_status status = nnp_convolution_kernel_gradient(
          algorithm, batchSize(), inputChannels(), outputChannels(),
          inputSize(), inputPadding(), kernelSize(), input.data(),
          outputGradient.data(), kernelGradient.data(),
          scratchSize == 0 ? nullptr : scratchBuffer.data(),
          scratchSize == 0 ? nullptr : &scratchSize, nnp_activation_identity,
          NULL, this->threadpool, NULL);
      ASSERT_EQ(nnp_status_success, status);

      const float maxError = std::inner_product(
          referenceKernelGradient.cbegin(), referenceKernelGradient.cend(),
          kernelGradient.cbegin(), 0.0f,
          [](float x, float y) -> float { return std::max<float>(y, x); },
          relativeError);
      maxErrors.push_back(maxError);
    }
  ...
  void testInference(enum nnp_convolution_algorithm algorithm,
    ...
    std::vector<float> output(outputChannels() * outputHeight() *
                              outputWidth());
    std::vector<float> referenceOutput(outputChannels() * outputHeight() *
                                       outputWidth());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      nnp_convolution_output__reference(
          1, inputChannels(), outputChannels(), inputSize(), inputPadding(),
          kernelSize(), outputSubsampling(), input.data(), kernel.data(),
          bias.data(), referenceOutput.data(), this->threadpool);
      ...
      enum nnp_status status = nnp_convolution_inference(
          ...
          precompute ? nnp_convolution_transform_strategy_reuse
                     : nnp_convolution_transform_strategy_compute,
          inputChannels(), outputChannels(), inputSize(), inputPadding(),
          kernelSize(), outputSubsampling(), input.data(),
          static_cast<const float*>(kernelData), bias.data(), output.data(),
          scratchSize == 0 ? nullptr : scratchBuffer.data(),
          scratchSize == 0 ? nullptr : &scratchSize, activation, nullptr,
          this->threadpool, nullptr);
      ASSERT_EQ(nnp_status_success, status);
  ...
  void testDepthwiseInference(
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      nnp_convolution_depthwise_output__reference(
          ...
          outputChannels() / inputChannels(), inputSize(), inputPadding(),
          kernelSize(), outputSubsampling(), input.data(), kernel.data(),
          bias.data(), referenceOutput.data(), this->threadpool);
 ...
 private:
  inline static float relativeError(float reference, float actual) {
    return std::abs(reference - actual) /
           std::max(FLT_MIN, std::abs(reference));
  }

  inline static float median(std::vector<float>& array) {
    std::nth_element(array.begin(), array.begin() + array.size() / 2,
                     array.end());
